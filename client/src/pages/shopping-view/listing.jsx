...
function parseFiltersFromParams(searchParams) {
  ...
  if (subCategory) filters.subCategory = subCategory.split(",").filter(Boolean);
...
function sanitizeFilters(raw, { productCategories, brands }) {
  ...
  const validSub = new Set(
    (productCategories || []).flatMap((c) => (c.subCategories || []).map((s) => s.id))
  );
  ...
  if (out.subCategory) out.subCategory = out.subCategory.filter((id) => validSub.has(id));
  ...
  if (!out.subCategory?.length) delete out.subCategory;
...
function ShoppingListing() {
  const { brand, categoryOptionsMap, subCategoryOptionsMap, productCategories, brands } = useSiteSettings();
  ...
  useEffect(() => {
    ...
    if (hasUrl) {
      ...
      if (!searchParams.toString() && hasStored) {
        setFilters((prev) => (JSON.stringify(prev) === JSON.stringify(cleaned) ? prev : cleaned));
      } else {
  ...
  const activeSubCategories = filters?.subCategory || [];

  const pageTitle =
    activeSubCategories.length === 1
      ? subCategoryOptionsMap[activeSubCategories[0]] || "Shop All Drops"
