...
function SpotlightSection({ products = [], handleGetProductDetails, handleAddtoCart }) {
  ...
  const validFilters = Array.isArray(quickFilters) ? quickFilters.filter((q) => q && q.label) : [];
  const tabs = [allTab, ...validFilters];
  const safeIdx = activeIdx >= 0 && activeIdx < tabs.length ? activeIdx : 0;
  const active = tabs[safeIdx] || allTab;

  const filtered =
    !active || active.category === "all"
      ? (products || [])
      : (products || []).filter((p) => p && p.category === active.category);
