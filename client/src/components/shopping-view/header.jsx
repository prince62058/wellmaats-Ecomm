          ...
          {user ? (
            <>
              {user?.role === "admin" && (
                <button
                  type="button"
                  className={`${actionClass} font-semibold text-emerald-700 bg-emerald-50/70 hover:bg-emerald-100/70`}
                  onClick={() => closeAnd(() => navigate("/admin/dashboard"))}
                >
                  <LayoutDashboard className="w-5 h-5 text-emerald-700 shrink-0" />
                  Admin Panel
                </button>
              )}
